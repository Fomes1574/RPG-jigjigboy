...
window.toggleSidebarJogador = function(numSlot) {
    ...
    if (sidebar.classList.contains('sidebar-fechada')) {
        ...
        async function getMasterStatus() {
            const masterSnap = await safeGet('master');
            const master = masterSnap.val() || null;
            if(!master?.uid) {
                return { configured: false, uid: '', username: '', usernameKey: '' };
            }
            const profileSnap = await safeGet(`authProfiles/${master.uid}`);
            const profile = profileSnap.val() || {};
            return {
                configured: true,
                uid: master.uid,
                username: profile.username || master.usernameKey || '',
                usernameKey: master.usernameKey || profile.usernameKey || ''
            };
        }

        function renderMasterControlCard(masterStatus) {
            const uidAbreviado = masterStatus.uid ? `${masterStatus.uid.slice(0, 6)}...` : '-';
            return `
                <div style="border:2px solid #8b6d43; background:rgba(0,0,0,0.45); padding:18px; min-height:230px; box-shadow:inset 0 0 18px rgba(0,0,0,0.65);">
                    <h2 style="font-family:'Cinzel', serif; color:#d4af37; margin:0 0 12px;">Mestre</h2>
                    <p><strong>Estado:</strong> <span style="color:${masterStatus.configured ? '#d95757' : '#27ae60'};">${masterStatus.configured ? 'Configurado' : 'Livre'}</span></p>
                    <p><strong>Usuário vinculado:</strong> ${escapeHtml(masterStatus.username || 'Nenhum')}</p>
                    <p><strong>UID:</strong> ${escapeHtml(uidAbreviado)}</p>
                    <p style="font-size:12px; color:#9c8464; line-height:1.4;">Este botão libera o cargo de Mestre no app. A conta antiga pode continuar aparecendo em Authentication, mas não acessará o app sem perfil. Para reutilizar exatamente o mesmo nome de usuário, apague também o e-mail interno em Authentication.</p>
                    <button onclick="resetMasterControl()" style="width:100%; margin-top:14px; background:linear-gradient(to bottom,#8c1c13,#4a1111); border:1px solid #d95757; color:#fff; padding:12px; cursor:pointer; font-weight:bold;">Resetar login do Mestre</button>
                </div>
            `;
        }

        window.resetMasterControl = async function() {
            if(usuarioAtual?.role !== 'control') return;
            const status = await getMasterStatus();
            if(!status.configured || !status.uid) {
                mostrarMensagemControl('Mestre já está livre.');
                return;
            }
            const confirmacao = prompt('Digite APAGAR para resetar o login do Mestre.');
            if(confirmacao !== 'APAGAR') return;
            await safeRemove('master');
            await safeRemove(`authProfiles/${status.uid}`);
            if(status.usernameKey) await safeRemove(`usernames/${status.usernameKey}`);
            await window.renderControlSlots();
            mostrarMensagemControl('Login do Mestre foi resetado. Agora outro usuário pode criar acesso como Mestre.');
        }
        ...
        window.renderControlSlots = async function() {
            ...
            const [statuses, masterStatus] = await Promise.all([
                Promise.all(CHARACTER_SLOTS.map(slot => getFichaStatus(slot.id))),
                getMasterStatus()
            ]);
            const fichaCardsHtml = statuses.map(status => `
            ...
            `).join('');
            container.innerHTML = fichaCardsHtml + renderMasterControlCard(masterStatus);
        }
